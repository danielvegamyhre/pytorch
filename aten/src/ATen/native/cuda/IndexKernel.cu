  ...
  gpu_index_kernel(
      ...
      [] C10_DEVICE(
        ...
        *reinterpret_cast<scalar_t*>(out_data) =
            *reinterpret_cast<const scalar_t*>(in_data + offset);
      });
}

template <typename scalar_t>
void index_put_kernel_impl(
    TensorIterator& iter,
    const IntArrayRef index_size,
    const IntArrayRef index_stride) {
  gpu_index_kernel(
...
void index_put_kernel_quantized_cuda(
    TensorIterator& iter,
    const IntArrayRef index_size,
    const IntArrayRef index_stride,
    const bool accumulate,
  ...
  AT_DISPATCH_QINT_AND_SUB_BYTE_TYPES(iter.dtype(), "index_put", [&] {
    constexpr int64_t qmin =
        std::numeric_limits<typename scalar_t::underlying>::min();
    ...
    gpu_index_kernel(
        ...
        index_size,
        index_stride,
        [inv_scale, zero_point, qmin, qmax] C10_DEVICE(
            char* const out_data,
            const char* const in_data,
            const int64_t offset) {
          int64_t qvalue = static_cast<int64_t>(
              zero_point + nearbyintf(*(float*)in_data * inv_scale));
          // See https://github.com/pytorch/pytorch/issues/127666
          // and https://github.com/pytorch/pytorch/issues/128253.
          // hip-clang std::clamp __glibcxx_assert_fail host function when
          // building on Fedora40/gcc14. The following replaces
          // std::clamp(qvalue, qmin, qmax) and is a viable solution for both
          // CUDA and ROCm since std::clamp and this replacement generates the
          // same PTX. Using #ifdef USE_ROCM to differentiate caused Windows
          // build failures. The replacement should generate the same PTX as
          // std::clamp. See https://godbolt.org/z/Wde9KW3v4
  ...
  AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND3(
      ...
      [&] {
        ...
        AT_DISPATCH_INDEX_TYPES(
            cuda::detail::canUse32BitIndexMath(output) ? ScalarType::Int
                                                       : ScalarType::Long,
            "put_cuda_index",
            [&] {
              ...
              if (accumulate) {
                ...
                cuda_take_put_kernel<scalar_t, index_t>(
                    iter,
                    output,
...
void take_kernel(TensorIterator& iter, const TensorBase& input) {
  AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND3(
      ...
      [&] {
        ...
        // implemented
        AT_DISPATCH_INDEX_TYPES(
            cuda::detail::canUse32BitIndexMath(input) ? ScalarType::Int
                                                      : ScalarType::Long,
            "take_cuda_index",
            [&] {
...
void launch_masked_scatter_kernel(
  ...
  at::cuda::cub::mask_exclusive_sum(mask_data, maskPrefixSum_data, mask_numel);
      ...
      [&]() {
        ...
        gpu_kernel(
            ...
            [=] GPU_LAMBDA(
                const scalar_t a,
                const bool mask,
                const int64_t maskPrefixSum) -> scalar_t {
              if (mask) {
...
void flip_kernel(TensorIterator& iter, const bool quantized) {
  if (quantized) {
    AT_DISPATCH_QINT_AND_SUB_BYTE_TYPES(
        iter.dtype(), "flip_quantized_cuda", [&] {
          using dtype = OpaqueType<sizeof(scalar_t)>;
...
REGISTER_CUDA_DISPATCH(
    ...
    &index_put_kernel_quantized_cuda)
